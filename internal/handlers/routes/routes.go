...
import (
	...
	"ecommerce-product/internal/handlers/middlewares"
	"ecommerce-product/internal/pkg/healthcheck"
	"ecommerce-product/internal/pkg/product"
...
func NewRouter() {
	...
	productEndpoint := product.NewEndpoint()
	product := v1.Group("products")
	product.Get("/", productEndpoint.FindAll)
	product.Get("/ids", productEndpoint.FindAllByIDs)
	product.Post("/bulk", productEndpoint.BulkUpdateProducts)
	product.Get("/:id", productEndpoint.FindOne)
